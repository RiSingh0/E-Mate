...
<div class="padding-1">
              ...
              <div class="box-body">
                <br><br><br>
                <h4 style="margin-bottom: 10px;">PT 1 &emsp;Result :</h4>
                <?php        
                $sql="SELECT * from studentstbl ORDER BY st_id ASC";
                $result = mysqli_query($conn,$sql) or die("Error: " . mysqli_error($conn));
                while($resultArray = mysqli_fetch_array($result))
                {
                  $st_id=$resultArray['st_id'];
                  $name=$resultArray['st_fullname'];
                  $check=mysqli_query($conn,"SELECT * from pt1 where st_id='$st_id' or st_name='$name'"); 
                 
                  if($check=mysqli_query($conn,"SELECT * from pt1 where st_id='$st_id' or st_name='$name'"))
                  {
                  $checkrows=mysqli_num_rows($check);}else{$checkrows=0;}
                  if($checkrows<1)
                  {
                  $sql=" INSERT INTO pt1 (st_id,st_name)
                  VALUES ('$st_id','$name')";
                  ...
                  $sql="SELECT * from pt1 ORDER BY st_id ASC";
                  $result = mysqli_query($conn,$sql) or die("Error: " . mysqli_error($conn));
                  ...
                  while( $resultArray = mysqli_fetch_array($result))
                  {
                  $st_id1=$resultArray['st_id'];
                  $name1=$resultArray['st_name'];
                  echo  "<tr><td><label name='result' >Roll no &emsp; : &emsp; $st_id1 &emsp;|&emsp; Name &emsp; : &emsp; $name1 &emsp;| </label><input class='form-control form-white'display:none style='width: 10%; display: inline-block;' value='$st_id1' type='hidden' name='st_id[]'/><input class='form-control form-white' style='width: 10%; display: inline-block;' placeholder='subject1' type='text' name='sub1[]'/>|<input class='form-control form-white' style='width: 10%; display: inline-block;' placeholder='subject2' type='text' name='sub2[]'/>|<input class='form-control form-white' style='width: 10%; display: inline-block;' placeholder='subject3' type='text' name='sub3[]'/>|<input class='form-control form-white' style='width: 10%; display: inline-block;' placeholder='subject4' type='text' name='sub4[]'/>|<input class='form-control form-white' style='width: 10%; display: inline-block;' placeholder='subject5' type='text' name='sub5[]' />|<input class='form-control form-white' style='width: 10%; display: inline-block;' placeholder='subject6' type='text' name='sub6[]'/></td></tr>"; 
                  }
                  echo " </table><button type='submit' class='link btn btn-primary' name='save' id='save'>Update</button> <a  class='link btn btn-primary' href='pt1_result.php'>PT 1 RESULT</a>
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                  <a  class='link btn btn-primary delete_channel float-right'>Reset Result</a>";
                }
                else{
                    echo "<table id='' class='table table-bordered table-striped'><tr><td><label class='button'  name='nodata' >NO DATA FOUND!</label></td></tr><table>"; 
                }
...
?>
    ...
    { 
       for($j=0;$j<$k;$j++)
       {
        ...
        $sub6 = $_POST['sub6'][$j];
        $st_id = $_POST['st_id'][$j];
        $sql="UPDATE pt1 SET sub1 = '$sub1',sub2 = '$sub2',sub3 = '$sub3',sub4='$sub4',sub5='$sub5', sub6='$sub6' WHERE st_id = '$st_id'";
        if (mysqli_query($conn,$sql))
        {
